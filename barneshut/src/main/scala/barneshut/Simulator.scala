...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val bb = boundaries
    if (bb.minX <= body.x && body.x <= bb.maxX && bb.minY <= body.y && body.y <= bb.maxY) {
      boundaries
    } else {
      bb.minX = math.min(bb.minX, body.x)
      bb.maxX = math.max(bb.maxX, body.x)
      bb.minY = math.min(bb.minY, body.y)
      bb.maxY = math.max(bb.maxY, body.y)
      bb
    }
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val bb = new Boundaries
    bb.minX = math.min(a.minX, b.minX)
    bb.maxX = math.max(a.maxX, b.maxX)
    bb.minY = math.min(a.minY, b.minY)
    bb.maxY = math.max(a.maxY, b.maxY)
    bb
  }
  ...
  def updateSectorMatrixes(sectorMatrixes: SectorMatrix, body: Body): SectorMatrix =
    sectorMatrixes += body

  def mergeSectorMatrixes(a: SectorMatrix, b: SectorMatrix): SectorMatrix =
    a.combine(b)

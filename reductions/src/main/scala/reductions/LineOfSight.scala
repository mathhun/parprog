...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    for (i <- 0 until input.length) {
      val tan: Float = if (i == 0) 0f else input(i) / i
      mx = max(mx, tan)
      output(i) = mx
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    for (i <- from until until) {
      val tan = if (i == 0) 0f else input(i) / i
      mx = max(mx, tan)
    }
    mx
  }
  ...
  def downsweepSequential(
    input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int
  ): Unit = {
    var mx = startingAngle
    for (i <- from until until) {
      val tan = if (i == 0) 0f else input(i) / i
      mx = max(mx, tan)
      output(i) = mx
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      case leaf: Leaf =>
        downsweepSequential(input, output, startingAngle, leaf.from, leaf.until)
      case node: Node =>
        parallel(
          downsweep(input, output, startingAngle, node.left),
          downsweep(input, output, max(node.left.maxPrevious, startingAngle), node.right)
        )
    }
  }
